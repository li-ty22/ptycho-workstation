...
__global__ void addToArrayBI_c(float const * sarray, float * larray, 
    ...
    if (idx < Np_px - 1 & idy < Np_py - 1){
        ...
        float sarray_val_ul, sarray_val_bl, sarray_val_ur, sarray_val_br;
        for (int iSlice = 0; iSlice < nSlices; iSlice++) {
            for (int iSpot = 0; iSpot < nSpotsParallel; iSpot++) { 
                ...
                atomicAdd(&larray[id_large], (sarray_val_ul*ul[iSpot] + sarray_val_bl*bl[iSpot] + sarray_val_ur*ur[iSpot] + sarray_val_br*br[iSpot]));
            }
        }
    }
}
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    const float * p_projection = (float *)mxGPUGetDataReadOnly(m_projection);
