...
__global__ void cropArray_c(float * sarray, float const * larray,  
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    // --- create mxGPUArray ---
    mxGPUArray * m_projection = mxGPUCopyFromMxArray(prhs[0]);
    float * p_projection = (float *)mxGPUGetData(m_projection);

    const mxGPUArray * m_object = mxGPUCreateFromMxArray(prhs[1]);
    const float * p_object = (float *)mxGPUGetDataReadOnly(m_object);
    ...
    cropArray_c<<<dimBlock, dimThread>>>(p_projection, p_object, 
    ...
    plhs[0] = mxGPUCreateMxArrayOnGPU(m_projection);
